...
pub fn write_tiff(
    ...
    let geo_key_directory = if qmt.crs == CRS::Epsg4326 {
        vec![
            ...
            4096, 0, 1, 4979, // VerticalGeoKey = 4979 (WGS 84 3D) (used here to document use of ellipsoidal height)
    ...
    image
        .encoder()
        .write_tag(Tag::ModelTiepointTag, &tiepoints[..]).unwrap();

    image
        .encoder()
        .write_tag(Tag::GeoKeyDirectoryTag, &geo_key_directory[..]).unwrap();
