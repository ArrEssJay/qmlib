...
fn main() -> Result<(), String> { // Update Result to specify the error type
    ...
    match tile::load_quantized_mesh(&path) {
        Ok(tile) => {
            ...
            println!("\n\nVertex Count (U): {:#?}", tile.quantized_mesh.vertex_data.u.len());
            println!("Vertex Count (V): {:#?}", tile.quantized_mesh.vertex_data.v.len());
            println!("Vertex Count (Height): {:#?}", tile.quantized_mesh.vertex_data.height.len());

            println!("\n\nTriangle Count: {:#?}", tile.quantized_mesh.vertex_data.triangle_count);
            println!("Triangle Long Index: {:#?}", tile.quantized_mesh.vertex_data.index_data_long.is_some());
            println!("Triangle Short Index: {:#?}", tile.quantized_mesh.vertex_data.index_data_short.is_some());
            

            println!("\nLong Edge Index: {:#?}", tile.quantized_mesh.vertex_data.edge_indices_long.is_some());
            println!("Short Edge Index: {:#?}", tile.quantized_mesh.vertex_data.edge_indices_short.is_some());

            if let Some(edge_indices_long) = &tile.quantized_mesh.vertex_data.edge_indices_long {
                // Now you can use edge_indices_long safely
                // For example:
                println!("East Edge Vertex Count: {:?}", edge_indices_long.east_vertex_count);
                println!("West Edge Vertex Count: {:?}", edge_indices_long.west_vertex_count);
                println!("North Edge Vertex Count: {:?}", edge_indices_long.north_vertex_count);
                println!("South Edge Vertex Count: {:?}", edge_indices_long.south_vertex_count);
            ...
            if let Some(edge_indices_short) = &tile.quantized_mesh.vertex_data.edge_indices_short {
                // Now you can use edge_indices_long safely
                // For example:
                println!("East Edge Vertex Count: {:?}", edge_indices_short.east_vertex_count);
                println!("West Edge Vertex Count: {:?}", edge_indices_short.west_vertex_count);
                println!("North Edge Vertex Count: {:?}", edge_indices_short.north_vertex_count);
                println!("South Edge Vertex Count: {:?}", edge_indices_short.south_vertex_count);
